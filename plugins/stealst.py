...
async def register_commands():
                ...
                while not success and pack_num <= 5:
                    ...
                    if "Invalid pack selected" in resp.text or "choose the sticker pack" not in resp.text.lower():
                        ...
                        if cmd_new == "/newemojipack" and "choose the type" in r_type.text.lower():
                            try:
                                await r_type.click(text=pack_type_msg)
                            except Exception:
    ...
    async def kang_pack(event):
        ...
        if input_str and ("t.me/addstickers/" in input_str or "t.me/addemoji/" in input_str):
            match = re.search(r'(?:https?://)?t\.me/(?:addstickers|addemoji)/([a-zA-Z0-9_]+)', input_str)
            if match:
                ...
                await event.reply("❌ **Invalid sticker/emoji pack link provided!**")
                return
