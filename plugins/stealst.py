...
async def register_commands():
    ...
    async def kang_sticker(event):
        ...
        clean_name = "".join(e for e in base_name if e.isalnum())
        
        pack_title = f"{base_name} @netcorexp"
            ...
            async with event.client.conversation("@Stickers", timeout=15) as conv:
                while not success and pack_num <= 5:
                    ...
                    if "Invalid pack selected" in resp.text or "choose the sticker pack" not in resp.text.lower():
                        ...
                        await conv.get_response() # How to call it
                        ...
                        if "Invalid pack" in r1.text or "does not exist" in r1.text:
                            ...
                            link = f"https://t.me/addstickers/{curr_short}"
                            success = True
                        elif "send me the sticker" in r1.text.lower():
                            await conv.send_file(downloaded_file, force_document=True)
    ...
    async def kang_pack(event):
        ...
        if not reply or not reply.document:
            await event.reply("❌ **Please reply to a sticker from the pack you want to steal!**")
            return
        ...
        user = await event.client.get_me()
        custom_name = event.pattern_match.group(1)
        base_name = custom_name.strip() if custom_name else pack.set.title
        clean_name = "".join(e for e in base_name if e.isalnum())
        ...
        is_anim = pack.set.animated
        is_vid = pack.set.videos
        ...
        limit = pack.set.count # Unlimited up to standard 120 pack max
        eta_seconds = limit * 2.5 # ~2.5s per sticker
        
        await status.edit(f"🔄 **Stealing pack:** `{pack.set.title}`\n"
        ...
        try:
            async with event.client.conversation("@Stickers", timeout=20) as conv:
                ...
                if "choose the sticker pack" not in resp.text.lower():
                    ...
                    if "Invalid pack" in r_choose.text or "does not exist" in r_choose.text:
                        ...
                        await conv.send_message(curr_title)
                        await conv.get_response() # Send sticker
                    elif "send me the sticker" not in r_choose.text.lower():
                        await conv.send_message("/cancel")
                ...
                for i, doc in enumerate(pack.documents[:120]): # 120 is max allowed per pack
                    ...
                    # Download
                    downloaded_file = await event.client.download_media(doc, "kang_pack_media")
            ...
            await event.client.send_message("@Stickers", "/cancel")
            await status.edit("❌ **Error:** `@Stickers` bot took too long to respond during bulk steal. Saved what was done and cancelled the rest.")
            return
        except Exception as e:
            await event.client.send_message("@Stickers", "/cancel")
